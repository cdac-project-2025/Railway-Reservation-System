...
import com.railway.dto.UserDetailsDto;
...
public class UserServiceImpl implements UserService {
	...
	public ApiResponse updateUser(UserRegDto userDto, Long id) {
		...
		if(!userDto.getEmail().equals(user.get().getEmail()) && userDao.existsByEmail(userDto.getEmail()))
			throw new FieldNotAvailableException("Email already exists!");
		
		if(!userDto.getUserName().equals(user.get().getUserName()) && userDao.existsByUserName(userDto.getUserName()))
			throw new FieldNotAvailableException("Username already exists!");
	...
	public UserDetailsResponse getUserDetails(Long id) {
		Optional<User> user = userDao.findById(id);
		UserDetailsDto details = modelMapper.map(user.get(), UserDetailsDto.class);
		
		return new UserDetailsResponse("success", details);
	}
}
